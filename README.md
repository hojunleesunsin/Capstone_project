...
- [프로젝트 결론](#프로젝트-결론)
- [프로젝트 문제점](#프로젝트-문제점)
- [학습 모델 및 예측 모델 설명](#학습-모델-및-예측-모델-설명)
...
## 프로젝트 결론
처음에 의도 하였던대로 기존에 있던 제품들과는 다르게 아기의 음성이 인식되면 아기의 울음소리에 대해 서버에서 이에 대해 분류하고 실시간성이 있게끔 잘 구현이 되었습니다.  
기존의 제품과의 차이점 이외에도 온습도 값을 측정하여 서버에서 받아서 데이터를 처리하여 안드로이드 앱으로 보내어 알림을 띄우고 울음소리가 인식되면 백색 소음을 발생시키고 모터로 침대의 흔들림을 제어하는 것 역시 잘 구현이 되었고 안드로이드 앱에서 실시간으로 카메라로 실시간 스트리밍이 되는 기능 역시 잘 구현이 되었고 처음 기획하였던 대로 모든 기능들이 잘 구현이 되었습니다.  
<br>
...
- 아기의 음성 데이터셋을 확보하는 것에 어려움을 겪었고 데이터셋의 정확성이 배고픔 이외에는 정확하지 않아서 아기의 상태를 다중분류 하는 것에서 배고프거나 그 외의 분류로 변경하여 진행하였습니다.

<br>

## <a href = "https://github.com/kdk0411/Audio_Classification_Model">학습 모델 및 예측 모델 설명</a>